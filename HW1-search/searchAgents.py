...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # use a tuple to store position, and running list of indexes visited
    ...
    def isGoalState(self, state):
        ...
        if state[0] in self.corners:
            if state[0] not in list(state1temp):
                state1temp.append(state[0])
        if (len(state1temp) == 4):
            return True
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx, nexty) not in corners_visited and (nextx, nexty) in self.corners:
                    var1 = corners_visited
                    var1.append((nextx, nexty))
                    tup = ((nextx, nexty), var1)
                    cost = 1
                    successors.append((tup, action, cost))
                else:
                    tup = ((nextx, nexty), corners_visited)
